...
ANCOMBC, ALDEx2 and DESeq2 are selected as the top 3 best performing tools for 
...
Phylum_sig_ancombc <- Phylum_ancom_res %>% 
  filter(q_val < 0.05) %>%  # adjusted p-values
  arrange(order(gtools::mixedorder(ASV)))

Phylum_ancombc_marker <- Phylum_sig_ancombc %>% 
  mutate(enrich_group = case_when(
    lfc >=0 ~ "Drought",
    lfc <0 ~ "Control"
  )) %>% 
...
Class_sig_ancombc <- Class_ancom_res %>% 
  filter(q_val < 0.05) %>%  # adjusted p-values
  arrange(order(gtools::mixedorder(ASV)))

Class_ancombc_marker <- Class_sig_ancombc %>% 
  mutate(enrich_group = case_when(
    lfc >=0 ~ "Drought",
    lfc <0 ~ "Control"
  )) %>% 
...
Order_sig_ancombc <- Order_ancom_res %>% 
  filter(q_val < 0.05) %>%  # adjusted p-values
  arrange(order(gtools::mixedorder(ASV)))

Order_ancombc_marker <- Order_sig_ancombc %>% 
  mutate(enrich_group = case_when(
    lfc >=0 ~ "Drought",
    lfc <0 ~ "Control"
  )) %>% 
...
Family_sig_ancombc <- Family_ancom_res %>% 
  filter(q_val < 0.05) %>%  # adjusted p-values
  arrange(order(gtools::mixedorder(ASV)))

Family_ancombc_marker <- Family_sig_ancombc %>% 
  mutate(enrich_group = case_when(
    lfc >=0 ~ "Drought",
    lfc <0 ~ "Control"
  )) %>% 
...
Genus_sig_ancombc <- Genus_ancom_res %>% 
  filter(q_val < 0.05) %>%  # adjusted p-values
  arrange(order(gtools::mixedorder(ASV)))

Genus_ancombc_marker <- Genus_sig_ancombc %>% 
  mutate(enrich_group = case_when(
    lfc >=0 ~ "Drought",
    lfc <0 ~ "Control"
  )) %>% 
...
```{r intersections}
ASV_intersection <- list(Wilcox = sig_wilcox_r$ASV, DESeq2 = deseq_marker$ASV, edgeR = edger_marker$ASV, ALDEx2 = aldex_marker$ASV, ANCOMBC = ancombc_marker$ASV)
top3_ASV_intersection <- list(DESeq2 = deseq_marker$ASV, ALDEx2 = aldex_marker$ASV, ANCOMBC = ancombc_marker$ASV)
Phylum_intersection <- list(DESeq2 = Phylum_deseq_marker$Phylum, ALDEx2 = Phylum_aldex_marker$Phylum, ANCOMBC = Phylum_ancombc_marker$Phylum)
Class_intersection <- list(DESeq2 = Class_deseq_marker$Class, ALDEx2 = Class_aldex_marker$Class, ANCOMBC = Class_ancombc_marker$Class)
Order_intersection <- list(DESeq2 = Order_deseq_marker$Order, ALDEx2 = Order_aldex_marker$Order, ANCOMBC = Order_ancombc_marker$Order)
Family_intersection <- list(DESeq2 = Family_deseq_marker$Family, ALDEx2 = Family_aldex_marker$Family, ANCOMBC = Family_ancombc_marker$Family)
Genus_intersection <- list(DESeq2 = Genus_deseq_marker$Genus, ALDEx2 = Genus_aldex_marker$Genus, ANCOMBC = Genus_ancombc_marker$Genus)
```
...
```{r Order UpsetPlot between top3 DAA Methods, error=FALSE, message=FALSE, fig.height=5, fig.width=6, fig.cap="**Order Intersections between different DAA Tools.** Upset plots displaying the overlap and uniqueness of significant orders identified by top 3 DAA methods (ALDEx2, DESeq2, ANCOMBC). The horizontal bars show the total number of orders for each tool, while the vertical bars show the number of shared orders between corresponding sets, sorted by the total number of shared orders. All tools use an alpha threshold of 0.05 for significance"}
